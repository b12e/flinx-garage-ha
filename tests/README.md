...
| `test_ble_state.py` | The reply to a BLE command decodes to the same attribute report MQTT carries, and drives the door position, LED and cycle counter. Uses real frames from a debug log, including a chunked notification. |
| `test_command_errors.py` | A command that never reached the door is reported as failed rather than silently succeeding, carrying the cloud gateway's own wording (e.g. "Too frequent operation"). |
