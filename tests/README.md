...
| `test_ble.py` | Which BLE peripheral a door resolves to: bound address, a name ending in the device code, the single-door prefix fallback, and cloud-only otherwise. Asserts two doors never resolve to the same peripheral. |
...
| `test_flows.py` | Config flow (credential errors, unique ID normalisation, door selection) and options flow (device add/remove including registry cleanup, Bluetooth binding). |
